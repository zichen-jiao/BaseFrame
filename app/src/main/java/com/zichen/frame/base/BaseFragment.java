...
//    }
    ...
    /**
     * 初始化UI控件或者不需要通过Lazy方式加载Fragment时，可以将数据初始化等操作放在这
     *
     * @return
     */
    ...
    /**
     * 通过Lazy方式加载的Fragment时，将数据初始化等操作放在这里
     */
    ...
    protected void showToastySuccess(CharSequence sequence) {
        Toasty.success(getActivity(), sequence).show();
    }

    protected void showToastyError(CharSequence sequence) {
        Toasty.error(getActivity(), sequence).show();
    }

    protected void showToastyNormal(CharSequence sequence) {
        Toasty.normal(getActivity(), sequence).show();
    }
